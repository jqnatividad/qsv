...
[![Minimum supported Rust version](https://img.shields.io/badge/Rust-1.67.1-red?logo=rust)](#minimum-supported-rust-version)
...
| [foreach](/src/cmd/foreach.rs#L3)<br>❇️ | Loop over a CSV to execute shell commands. (not available on Windows)  |
...
| [joinp](/src/cmd/joinp.rs#L2)<br>❇️🚀🐻‍❄️ | Inner, left, outer, cross, anti & semi joins using the [Pola.rs](https://www.pola.rs) engine. Unlike `join`, it can process files larger than RAM and is multi-threaded. However, it cannot do case-insensitive joins. |
...
It also allows mapping of multiple new computed columns, supports random access with indexed CSV files, and has [several helper functions](https://github.com/jqnatividad/qsv/blob/929f17e4ee50fb9f3a327708bbefd40565ca7b4c/src/cmd/luau.rs#L1250-L1931) to help ease the development of [full-fledged data-wrangling scripts](https://github.com/jqnatividad/qsv/blob/4e521b177ea3a6a06c83222458bb1349a67606f4/tests/test_luau.rs#L524-L571).

As date manipulation is often needed, we're also preloading the [LuaDate](https://tieske.github.io/date/) module.

Finally, as qsv's DSL, `luau` will gain even more features over time compared to the `python` feature.
