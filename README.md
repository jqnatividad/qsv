...
| <a name="snappy_deeplink"></a>[snappy](/src/cmd/snappy.rs#L2)<br>🚀 | Does streaming compression/decompression of the input using the [Snappy](https://google.github.io/snappy/) format.<br>Compresses NYC's 311 data (15 gb, 28m rows) to 5.4 gb in 6.67 seconds with the multithreaded `compress` subcommand (*2.25 gb/sec* with a 0.36 (2.78:1) compression ratio).<br>Compare that to zip 3.0, which compressed the same file to 2.9 gb in 248.3 seconds (37x slower at 0.06 gb/sec with a 0.19 (5.17:1) compression ratio - for just an additional 17% (2.5 gb) of saved space). |
