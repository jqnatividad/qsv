...
| [joinp](/src/cmd/joinp.rs#L2)<br>✨🚀🐻‍❄️ | Inner, left, outer, cross, anti & semi joins using the [Pola.rs](https://www.pola.rs) engine. Unlike `join`, it can process files larger than RAM and is multi-threaded. However, it cannot do case-insensitive joins. |
...
| <a name="snappy_deeplink"></a>[snappy](/src/cmd/snappy.rs#L2)<br>🚀 | Does streaming compression/decompression of the input using Google's [Snappy](https://google.github.io/snappy/) format.<br>Compresses NYC's 311 data (15 gb, 28m rows) to 5.4 gb in 6.67 seconds with the multithreaded `compress` subcommand - *2.25 gb/sec* with a 0.36 (2.78:1) compression ratio.<br>Compare that to [zip 3.0](https://infozip.sourceforge.net/Zip.html), which compressed the same file to 2.9 gb in 248.3 seconds - 37x slower at 0.06 gb/sec with a 0.19 (5.17:1) compression ratio - for just an additional 17% (2.5 gb) of saved space. |
...
| [validate](/src/cmd/validate.rs#L2)<br>📇🚀 | Validate CSV data blazingly-fast (e.g. *up to 350,000 rows/second* using NYC's 311 schema) using [JSON Schema Validation](https://json-schema.org/draft/2020-12/json-schema-validation.html) (See `schema` command) & put invalid records into a separate file with an accompanying detailed validation error report file. If no JSON schema file is provided, validates if a CSV conforms to the [RFC 4180 standard](https://datatracker.ietf.org/doc/html/rfc4180). |
...
Similarly, if the `--output` file has an extended CSV/TSV ".sz" extension, qsv will automatically do streaming compression as it writes it (See the [`snappy`](/src/cmd/snappy.rs#L2) command for 4-5x faster
...
[Luau 0.571](https://github.com/Roblox/luau/releases/tag/0.571) is currently embedded - qsv's policy is to use the latest stable Luau version at the time of each qsv release.
...
| `QSV_MAX_JOBS` | number of jobs to use for multithreaded commands (currently `apply`, `dedup`, `extsort`, `frequency`, `schema`, `sort`, `split`, `stats`, `to`, `tojsonl` & `validate`). If not set, max_jobs is set to the detected number of logical processors.  See [Multithreading](docs/PERFORMANCE.md#multithreading) for more info. |
