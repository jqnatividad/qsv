...
The `--select` option and several commands (`apply`, `applydp`, `schema`, `search`, `searchset`, `select`, `sqlp` & `replace`) allow the user to specify regular expressions. We use the [`regex`](https://docs.rs/regex) crate to parse, compile and execute these expressions. [^3]
