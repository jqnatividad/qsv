...
run apply_op_string "$bin_name" apply operations lower Country -q "$data"
run apply_op_string_ascii "$bin_name" apply operations asciilower Country -q "$data"
run apply_op_similarity "$bin_name" apply operations lower,simdln Country --comparand union --new-column Country_sim-union_score -q "$data"
run apply_op_soundex "$bin_name" apply operations lower,soundex City --comparand boston --new-column City_boston_soundex -q "$data" 
run --nyc311 apply_datefmt "$bin_name" apply datefmt \"Created Date\" -q "$nyc311data"
run --nyc311 apply_emptyreplace "$bin_name" apply emptyreplace \"Bridge Highway Name\" --replacement Unspecified -q "$nyc311data"
...
run lua "$bin_name" lua map pop_empty "tonumber\(Population\)==nil" -q "$data"
run partition "$bin_name" partition Region /tmp/partitioned "$data"
run --nyc311 pseudo "$bin_name" pseudo 'Unique Key' "$nyc311data"
...
run --index scramble_index "$bin_name" scramble -q "$data"
