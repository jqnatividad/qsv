static USAGE: &str = r#"
Fetchpostj fetches data from web services for every row using HTTP Post.
As opposed to fetchpost, it sends data as `application/json` instead of
`application\x-www-form-urlencoded`.

Fetchpostj is integrated with `jql` to directly parse out values from an API JSON response.
...
Fetchpostj caches responses to minimize traffic and maximize performance. By default, it uses
a non-persistent memoized cache for each fetchpostj session.
...
Note that the default values are the same as the fetch command, except fetchpostj creates the
cache at database 3, as opposed to database 1 with fetch and database 2 with fetchpostj.
...
Given the data.csv above, fetch the JSON response.
  ...
  $ qsv fetchpostj URL zipcode,country data.csv 
...
$ qsv fetchpostj URL zipcode,country --new-column form --jql '"form"' data.csv > data_with_response.csv
...
Alternatively, since we're using the same URL for all the rows, we can just pass the url directly on the command-line.
  ...
  $ qsv fetchpostj https://httpbin.org/post 2,3 --new-column form --jqlfile form.jql data.csv > data_with_formdata.csv
...
$ qsv fetchpostj https://httpbin.org/post col1-col3 data.csv -H "X-Api-Key:TEST_KEY" -H "X-Api-Secret:ABC123XYZ"

For more extensive examples, see https://github.com/jqnatividad/qsv/blob/master/tests/test_fetch.rs.

Usage:
    qsv fetchpostj (<url-column> <column-list>) [--jql <selector> | --jqlfile <file>] [--http-header <k:v>...] [options] [<input>]
    qsv fetchpostj --help
...
Fetchpostj options:
    ...
    --jsoninput                The input file is in JSON format, not CSV.     
    ...
    --rate-limit <qps>         Rate Limit in Queries Per Second (max: 1000). Note that fetch
                               ...
                               otherwise your fetchpostj job may look like a Denial Of Service attack.
    ...
    --report <d|s>             Creates a report of the fetchpostj job. The report has the same name as the
...
// connect to Redis at localhost, using database 3 by default when --redis is enabled
static DEFAULT_FPJ_REDIS_CONN_STR: &str = "redis://127.0.0.1:6379/3";
static DEFAULT_FPJ_REDIS_TTL_SECS: u64 = 60 * 60 * 24 * 28; // 28 days in seconds
static DEFAULT_FPJ_REDIS_POOL_SIZE: u32 = 20;
static TIMEOUT_FPJ_SECS: OnceCell<u64> = OnceCell::new();

const FETCHPOSTJ_REPORT_PREFIX: &str = "qsv_fetchpj_";
const FETCHPOSTJ_REPORT_SUFFIX: &str = ".fetchpostj-report.tsv";
...
impl RedisConfig {
    fn load() -> Self {
        Self {
            conn_str:      std::env::var("QSV_FPJ_REDIS_CONNSTR")
                .unwrap_or_else(|_| DEFAULT_FPJ_REDIS_CONN_STR.to_string()),
            max_pool_size: std::env::var("QSV_REDIS_MAX_POOL_SIZE")
                .unwrap_or_else(|_| DEFAULT_FPJ_REDIS_POOL_SIZE.to_string())
                .parse()
                .unwrap_or(DEFAULT_FPJ_REDIS_POOL_SIZE),
            ttl_secs:      std::env::var("QSV_REDIS_TTL_SECS")
                .unwrap_or_else(|_| DEFAULT_FPJ_REDIS_TTL_SECS.to_string())
                .parse()
                .unwrap_or(DEFAULT_FPJ_REDIS_TTL_SECS),
...
pub fn run(argv: &[&str]) -> CliResult<()> {
    ...
    TIMEOUT_FPJ_SECS.set(args.flag_timeout).unwrap();
    ...
    let client_timeout = time::Duration::from_secs(*TIMEOUT_FPJ_SECS.get().unwrap_or(&30));
    ...
    if report == ReportKind::None {
        ...
        report_wtr = Config::new(&Some(report_path.clone() + FETCHPOSTJ_REPORT_SUFFIX)).writer()?;
        ...
        let rptcol_prefix = if report == ReportKind::Detailed {
            FETCHPOSTJ_REPORT_PREFIX
        } else {
            ""
        };
        // the fetchpostj report has the following columns:
    ...
    let mut end_msg = format!(
        "{} records successfully fetchjposted as {}. {} errors.",
    ...
    if report != ReportKind::None {
        ...
        write!(
            &mut end_msg,
            " {} report created: \"{}{FETCHPOSTJ_REPORT_SUFFIX}\"",
            if report == ReportKind::Detailed {
                "Detailed"
            } else {
                "Short"
...
fn get_response(
    ...
    let timeout_secs = unsafe { *TIMEOUT_FPJ_SECS.get_unchecked() };
