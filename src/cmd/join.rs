...
impl<R: io::Read + io::Seek, W: io::Write> IoState<R, W> {
    ...
    fn full_outer_join(mut self) -> CliResult<()> {
            ...
            if let Some(rows) = validx.values.get(&key) {
                for &rowi in rows.iter() {
                    ...
                    match self.wsel2 {
                        Some(ref sel) => self
                            .wtr
                            .write_record(row1.iter().chain(sel.select(&scratch)))?,
        ...
        let mut m: AHashMap<_, _> = AHashMap::new();
            ...
            if !written {
                ...
                match self.wsel2 {
                    Some(ref sel) => {
                        ...
                        m = self.sel1.iter().zip(self.sel2.iter()).collect();
                        let row1 = (0..pad1.len()).map(|i| match m.get(&i) {
                            Some(j) => &scratch[**j],
                            None => b"",
                        });
    ...
    fn new_io_state(
        ...
        let wsel2: Option<Selection> = if self.flag_merge {
            Some(
                self.arg_columns2
                    .invert()
                    .selection(rdr2.byte_headers()?, !rconf2.no_headers)?,
            )
        } else {
            None
        };
