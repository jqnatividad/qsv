...
When debugging luajit, be sure to set the environment variable QSV_LOG_LEVEL=debug to see
...
pub fn run(argv: &[&str]) -> CliResult<()> {
            ...
            match computed_value {
                ...
                _ => {
                    return fail_clierror!(
                        "Unexpected value type returned by provided LuaJIT expression. \
                         {computed_value:?}"
                    );
                }
            }
    ...
    if let Some(epilogue) = args.flag_epilogue {
        ...
        let epilogue_string = match epilogue_value {
            mluajit::Value::String(string) => string.to_string_lossy().to_string(),
            mluajit::Value::Number(number) => number.to_string(),
            mluajit::Value::Integer(number) => number.to_string(),
            mluajit::Value::Boolean(boolean) => {
                (if boolean { "true" } else { "false" }).to_string()
            }
            mluajit::Value::Nil => String::new(),
            _ => {
                return fail_clierror!(
                    "Unexpected epilogue value type returned by provided LuaJIT expression. \
                     {epilogue_value:?}"
                );
            }
        };
